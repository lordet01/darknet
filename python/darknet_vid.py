...
def runOnVideo(net, meta, vid_source, thresh=.2, hier_thresh=.5, nms=.45):
        ...
        for j in range(num):
            for i in range(meta.classes):
                if dets[j].prob[i] > 0:
                    ...
                    cv2.putText(frame, (meta.names[i]).decode("utf-8"), 
				(x1, y1 + 20), 1, 1, classes_font_colors[0], 2, cv2.LINE_AA)
